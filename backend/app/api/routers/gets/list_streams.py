...
async def get_list_streams(data: Request, session: AsyncSession = Depends(get_async_session)):
    random_string = "".join(
    ...
    stream_settings = (
        update(UserStreamSettings)
        .where(UserStreamSettings.user_id == user_id)
        .values(stream_id=my_stream_id)
    )
