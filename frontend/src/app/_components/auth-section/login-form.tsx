...
export function Login() {
	...
	return (
		<form
			className="align-top flex flex-col rounded-xl w-full h-[65%] gap-7 mt-10"
			onSubmit={handleSubmit}
		>
			{logInForm.map((item) => (
				<div
					key={item.name}
					className=" flex gap-2 flex-col w-full mb-5 group border-b-0 [background:linear-gradient(#3b82f6_0_0)_bottom/var(--d,0)_3px_no-repeat] transition-all duration-500 [--d:0]"
				>
					<label
						htmlFor=""
