...
import { AUTH, BACKEND } from "@/shared/constants";
...
export function SignIn() {
	...
	return (
		<>
			<form
				className="flex flex-col rounded-xl w-full h-[65%] gap-7"
			...
			>
					...
					<div
						...
						className={clsx(
							"flex gap-2 flex-col w-full mb-5 group border-b-0 [background:linear-gradient(#3b82f6_0_0)_bottom/var(--d,0)_3px_no-repeat] transition-all duration-500 [--d:0]",
							{
								...
								"[background:linear-gradient(#477023_0_0)_bottom/var(--d,0)_3px_no-repeat]":
									(nickValid === true && item.name === "username") ||
									(emailValid === true && item.name === "email") ||
									(passValid === true && item.name === "password"),
								"[background:linear-gradient(#ff0000_0_0)_bottom/var(--d,0)_3px_no-repeat]":
									(nickValid === false && item.name === "username") ||
									(emailValid === false && item.name === "email") ||
									(passValid === false && item.name === "password"),
							},
						)}
					>
						<label
						...
						{item.name === "password" &&
							passValid === false &&
							form["password" as keyof FormStateType] !== "" && (
								<span className="text-red-500 text-xs absolute -bottom-4 left-0">
				...
				<div className="flex w-full items-center justify-center">
					<button
						className={
							roboto.className +
							" shadow-md h-[60px] w-[120px] text-[20px] font-stretch-125% font-medium rounded-full cursor-pointer"
						}
						type="submit"
					>
						Sign In
